...
===============================================================================
  ...
  CHANGE HISTORY:
    ...
    11 February 2017 -- adapted from lasoptimize for native LAS 1.4 extension
...
void usage(bool error=false, bool wait=false)
{
  ...
  fprintf(stderr,"lasproto -v -i las14.las\n");
  fprintf(stderr,"lasproto -v -i las14.lap -odix _dec -olas\n");
  fprintf(stderr,"lasproto -v -i *.las\n");
  fprintf(stderr,"lasproto -v -i *.lap -odix _dec -olas\n");
  fprintf(stderr,"lasproto -h\n");
  if (wait)
  {
...
*/
      ...
      U32 chunk_count = 25000 + (100 * (rand() & 0xFF)); 
...
//        lasindex.add(lasreader->point.get_x(), lasreader->point.get_y(), (U32)(laswriter->p_count));
        ...
        {
          laswriter->chunk();
          chunk_count = 25000 + (100 * (rand() & 0xFF)); 
        }
      }
